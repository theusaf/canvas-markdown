...
function showdownSpecialBlocks(): showdown.ShowdownExtension[] {
  ...
  function replacer(
    prefix: string,
    svg: string,
    type: keyof typeof specialBlockColors,
  ) {
    return `${prefix}<p class="cm-alert-${type}" style="display: flex; align-items: center; color: ${specialBlockColors[type]}">${createImage(
      svg,
  ...
  return [
    {
      ...
      replace: (_: string, prefix: string) =>
        replacer(prefix, noteSVG.innerHTML, "note"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!TIP]/,
      replace: (_: string, prefix: string) =>
        replacer(prefix, tipSVG.innerHTML, "tip"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!CAUTION]/,
      replace: (_: string, prefix: string) =>
        replacer(prefix, cautionSVG.innerHTML, "caution"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!WARNING]/,
      replace: (_: string, prefix: string) =>
        replacer(prefix, warningSVG.innerHTML, "warning"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!IMPORTANT]/,
      replace: (_: string, prefix: string) =>
        replacer(prefix, importantSVG.innerHTML, "important"),
    },
    {
      type: "output",
      regex: /<blockquote>\s*<p class="cm-alert-(\w+)"/gm,
      replace(text: string, type: keyof typeof specialBlockColors) {
