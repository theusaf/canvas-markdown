        ...
        if (footnotes) {
          for (const footnote of footnotes) {
            ...
            footnotesOutput.push(
              `<li class="footnote" value="${name}" id="${prefix}-ref-${name}">${content}</li>`,
            );
          }
        }
    ...
    // Inline footnotes
    {
      ...
      filter: (text) =>
        text.replace(
          /\[\^([\w]+)\]/gm,
          (str, name) =>
            `<a href="#${prefix}-ref-${name}"><sup>[${name}]</sup></a>`,
        ),
    },
  ];
}
  ...
  return [
    {
      type: "lang",
      regex: /(>\s*)\[!NOTE]/,
      replace: (_: string, prefix: string) => replacer(prefix, "🛈", "note"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!TIP]/,
      replace: (_: string, prefix: string) => replacer(prefix, "💡", "tip"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!CAUTION]/,
      replace: (_: string, prefix: string) => replacer(prefix, "🛑", "caution"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!WARNING]/,
      replace: (_: string, prefix: string) => replacer(prefix, "⚠", "warning"),
    },
    {
      type: "lang",
      regex: /(>\s*)\[!IMPORTANT]/,
      replace: (_: string, prefix: string) =>
        replacer(prefix, "🗪", "important"),
    },
    {
      type: "output",
