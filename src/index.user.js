...
function showdownSpecialBlocks() {
    ...
    function replacer(prefix, svg, color) {
        return `${prefix}<p class="cm-alert-note" style="display: flex; align-items: center; color: ${color}">${createImage(svg)}<span style="margin-left: 0.5rem;">Note</span></p>`;
    }
    const colors = {
        note: "#1f6fec",
        tip: "#228636",
    ...
    return [
        {
            type: "lang",
            regex: /(>\s*)\[!NOTE]/,
            replace: (_, prefix) => replacer(prefix, noteSVG.innerHTML, colors.note),
        },
        {
            type: "lang",
            regex: /(>\s*)\[!TIP]/,
            replace: (_, prefix) => replacer(prefix, tipSVG.innerHTML, colors.tip),
        },
        {
            type: "lang",
            regex: /(>\s*)\[!CAUTION]/,
            replace: (_, prefix) => replacer(prefix, cautionSVG.innerHTML, colors.caution),
        },
        {
            type: "lang",
            regex: /(>\s*)\[!WARNING]/,
            replace: (_, prefix) => replacer(prefix, warningSVG.innerHTML, colors.warning),
        },
        {
            type: "lang",
            regex: /(>\s*)\[!IMPORTANT]/,
            replace: (_, prefix) => replacer(prefix, importantSVG.innerHTML, colors.important),
        },
        {
            type: "output",
